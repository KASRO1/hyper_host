...
<main class="other_page">
            ...
            <div class="container other_page baza">
                 <div class="pages">
                    <a href="/">Главная</a> / <span>База знаний</span>
                 </div>
                 <div class="grid">
                    <div class="left">
                        <form action="{{route('base_search')}}">
                            <div class="search">
                                <input type="text" name="i" @if(isset($_GET['i'])) value="{{$_GET['i']}}" @endif placeholder="Поиск по базе знаний">
                                <button>Найти</button>
                            </div>
                        </form>
                        @if($active_cat_info) <p class="title">{{__($active_cat_info->name)}}}</p> @else <p class="title">Все разделы</p> @endif
                    ...
                    <div class="right">
                        ...
                        @if(count($popular) != 0)
                        <p class="title">Популярные статьи</p>
                        @endif
